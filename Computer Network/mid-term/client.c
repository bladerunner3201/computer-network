...
int main(void) {
	...
	if ((sock_verify = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		printf("Can not verify through peer to peer.\n");
		return 1;
	}
	...
	strcat(buffer_verify_test, MY_ID);

	/* Start application */

	printf("Type help for menual.\n\n");

	while(1) {

		rfds_copy = rfds;
		if ((retval = select(sock_verify + 1, &rfds_copy, 0, 0, 0)) < 0) {
			printf("Error occured [select function].\n");
			return 1;
		}
		...
		if (retval > 0) {
			...
			if (FD_ISSET(STDIN_FD, &rfds_copy) > 0) {
				...
				// del
				else if (strncmp(type, "delete", 3) == 0) {
					scanf("%s", name);
					scanf(" %d.%d.%d.%d", &dest_temp[0], &dest_temp[1], &dest_temp[2], &dest_temp[3]);
					for (i = 0; i < 4; i++) dest[i] = (unsigned char)dest_temp[i];
...
void do_add(char* name, unsigned char* dest, char* output) {
	...
	*checksum = checksum_generate(output, strlen(name));
	...
	printf("Error occured [network address].\n\n");
		return;
	}

	// Checksum error
	if (!checksum_verify(result, strlen(name))) {
		printf("Error occured [checksum].\n\n");
		return;
	}
...
void do_get(char* name, char* output) {
	...
	*checksum = checksum_generate(output, strlen(name));
	...
	if (head == NULL) {
		...
		new_node->next = NULL;
		head = new_node;
		printf("Saved new IPv4 address information.\n\n");
	}
	else {
		while(ptr != NULL) {
			if (strcmp(ptr->name, name) == 0) {
				...
				if (strcmp(ptr->ipv4_addr_string, result) == 0) {
					printf("Received IPv4 address is already known.\n\n");
				}
				else {
...
void do_verify(char* name) {
	...
	*(unsigned short*)(output + 2) = htons(name_len); // Length
	...
	if (result[0] != MSG_VERIFY_RESPONSE || result[1] != MSG_SUCCESS) {
		printf("The address saved previously is invalid.\n\n");
		return;
	}
	...
	// Success
	printf("The address saved previously is verified.\n\n");
...
void view_help(void) {
	...
	printf("\n-------------------------Command List--------------------------\n");
	printf("|                                                             |\n");
	printf("|               add, del, get, help, verify, exit             |\n");
	printf("|                                                             |\n");
	printf("---------------------------------------------------------------\n\n");
