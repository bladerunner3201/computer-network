...
ValuePair::ValuePair(char *s, const char *v)
...
void addVP(PatTree *tree, char *s, const char *v)
...

{
  ...
  */
  ...
  for (ptr = buf; (next = strchr(ptr, '$')); ptr = next+1) {
  ///}
    int size;
    ValuePair *entry;
    size = next - ptr;
    ...
    next = strchr(ptr, '$');
    size = next - ptr;
    entry = (ValuePair *) pairs.find((byte *)ptr, size);
    if (entry)
...
void display_error(char *operation)
